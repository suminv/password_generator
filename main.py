...
class App(CTk.CTk):
    def __init__(self):
        ...
        self.setting_frame.grid(row=2, column=0, padx=(20, 20), pady=(0, 20), sticky='nsew')

        self.password_length_slider = CTk.CTkSlider(master=self.setting_frame, from_=0, to=100, number_of_steps=100,
                                                    command=self.slider_event)
        self.password_length_slider.grid(row=1, column=0, columnspan=3, padx=(20, 20), sticky='ew')
        ...
        self.cb_digit_var = tkinter.StringVar()
        self.cb_digit = CTk.CTkCheckBox(master=self.setting_frame, text='0-9', variable=self.cb_digit_var,
                                        onvalue=digits, offvalue='')
        self.cb_digit.grid(row=2, column=0, padx=10)
    ...
    def get_characters(self):
        chars = ''.join(self.cb_digit_var.get() + self.cb_lower_var.get() + self.cb_upper_var.get() +
